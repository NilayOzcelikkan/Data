...
public class AppWithCh extends AbstractIntegerProblem implements ConstrainedProblem<IntegerSolution>
{
	...
	public AppWithCh() {
	    ...
	    //for (int i = 0; i < (numOfUserStories * numOfSprints); i++) {
	    ...
	    //Her Yij değeri, i'nin noOfAffinity'sinden küçük veya eşit olmalıdır.
	    
	    int lineSum = 0;
	    //int maxNoOfAffinity = 0;
		...
		for(int i = 0 ; i < affinitySet.size() ; i++) {
		   ...
		   /*if (lineSum > maxNoOfAffinity) {
			   maxNoOfAffinity = lineSum;
		   }*/
		   lineSum = 0;
		}		
		int lowerBound = 0;
		int upperBound = numOfUserStories;
		//int upperBound = maxNoOfAffinity;
	    
	    //for (int i = (numOfUserStories * numOfSprints); i < getNumberOfVariables(); i++) {
	    for (int i = numOfUserStories; i < getNumberOfVariables(); i++) {
	    	lowerLimit.add(lowerBound);
	    	upperLimit.add(upperBound);
	    } 
	    
	   /* int affVar = 0;
	    for (int i = numOfUserStories; i < getNumberOfVariables(); i = i+numOfSprints) {
	    	for (int j = 0; j < numOfSprints; j++) {
	    		upperBound = noOfAffinity.get(affVar);
		    	lowerLimit.add(lowerBound);
	    		upperLimit.add(upperBound);
	    	}
	    	affVar++;
	...
	public void evaluate(IntegerSolution solution){
		...
		for(int j=0;j<numOfSprints;j++){
			for(int i=0;i<numOfUserStories;i++){	
				//if (solution.getVariableValue((i*numOfSprints)+j).intValue() == 1);
	    ...
	    */
	    	...
	    	for (int j = 0; j < numOfSprints; j++) { 
	    		for (int k = 0; k < numOfUserStories; k++) {
	    			for (int l = j+1; l < numOfSprints; l++) { 
	    				...
	    				firstPart = tempOR.get(k) * userStories.get(k).get(l);
	    				//secondPart = bigM * (1 - solution.getVariableValue((i*numOfSprints)+j).intValue());
	    				secondPart = bigM * (1 - userStories.get(i).get(j));
	    	...
	    	for (int j = 0; j < numOfSprints; j++) { 
	    		for (int k = 0; k < numOfUserStories; k++) {
	    			for (int l = j+1; l < numOfSprints; l++) { 
	    				...
	    				totalAndDependency += (tempAND.get(k) * userStories.get(k).get(l)) - (bigM * (1 - userStories.get(i).get(j)));
	    			}
	    		}	
	    ...
	    for (int i = 0; i < numOfUserStories; i++) {
	    	...
	    	for (int j = 0; j < numOfSprints; j++) { 
	    		...
	    		constraints[ (numOfUserStories*numOfSprints) + numOfSprints + (3*numOfUserStories) + (i*numOfSprints) + j] = sumOfAff - solution.getVariableValue(numOfUserStories+(i*numOfSprints)+j).intValue();
	    ...
	    for (int i = 0; i < numOfUserStories; i++) {
	    	for (int j = 0; j < numOfSprints; j++) { 
	    		...
	    		fPart = noOfAffinity.get(i) * userStories.get(i).get(j);
	    		//sPart = solution.getVariableValue(((numOfUserStories*numOfSprints)+numOfSprints)+(i*numOfSprints)+j).intValue();
	    		sPart = solution.getVariableValue(numOfUserStories+(i*numOfSprints)+j).intValue();
